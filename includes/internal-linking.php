...
function smart_ai_linker_generate_internal_links($post_ID, $post = null, $update = null)
{
    ...
    // Only process published posts and pages, or when status is 'future' (scheduled)
    if (!in_array($post->post_status, ['publish', 'future'])) {
        error_log('[Smart AI] Skipping - post status is ' . $post->post_status . ' (only publish/future are processed)');
        return $post_ID;
    }
